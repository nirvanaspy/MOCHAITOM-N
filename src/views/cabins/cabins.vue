  ...
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey' :data="listA" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column align="center" width="100px" :label="$t('table.deployBindDetail')">
        <template slot-scope="scope">
          <router-link class="pan-btn tiffany-btn" :to='{name:"cabinDetails",params:{id:scope.row.id}}'>查看</router-link>
        </template>
      </el-table-column>
...
<script>
  import { getCabins, addCabins, updateCabins, deleteCabins } from '@/api/cabins'
  ...
  export default {
    ...
    data() {
      const validateCabinName = (rule, value, callback) => {
        if (!isvalidUsername(value)) {
          callback(new Error('请输入正确的舱室名称'))
        } else {
          callback()
        }
      };
      return {
        ...
        pvData: [],
        rules: {
          name: [{ required: true, message: '请输入正确的舱室名称', trigger: 'blur'}]
    ...
    methods: {
      getList() {
        ...
        getCabins(this.proId,this.userData).then(response => {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            addCabins(formData, projectId, this.userData).then(() => {
      ...
      updateData() {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            let cabinData = qs.stringify(data);
            updateCabins(cabinData, id, this.userData).then(() => {
      ...
      handleDelete(row) {
        ...
        this.$confirm('确认删除吗？', '提示', {
          ...
          deleteCabins(id, this.userData).then(() => {
            this.$notify({
              title: '成功',
              message: '删除成功',
              type: 'success',
